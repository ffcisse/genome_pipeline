...
counts and lifestyle/lineage labels.
...
def per_genome_stats(df: pd.DataFrame) -> pd.DataFrame:
    props = numeric_property_columns(df)
...
def main():
    ...
    protein_stats = per_genome_stats(protein_df)
    cds_stats = per_genome_stats(cds_df)
