#!/usr/bin/env bash
#
# submit_phase4.sh -- submit run_phase4.sbatch with account/QOS/partition/
...
# read), so switching clusters or allocations is a config edit, not a
# script edit.
#
# Usage: workflow/scripts/submit_phase4.sh
...
echo "Submitting with: sbatch ${SBATCH_ARGS[*]} workflow/scripts/run_phase4.sbatch"
sbatch "${SBATCH_ARGS[@]}" workflow/scripts/run_phase4.sbatch
