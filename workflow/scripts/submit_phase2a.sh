#!/usr/bin/env bash
#
# submit_phase2a.sh -- submit run_phase2a.sbatch with account/QOS/partition/
# mail-user read from config/config.yaml's `slurm:` block (the same block
# submit_phase1.sh reads), so switching clusters or allocations is a config
# edit, not a script edit.
#
# Usage: workflow/scripts/submit_phase2a.sh
...
echo "Submitting with: sbatch ${SBATCH_ARGS[*]} workflow/scripts/run_phase2a.sbatch"
sbatch "${SBATCH_ARGS[@]}" workflow/scripts/run_phase2a.sbatch
