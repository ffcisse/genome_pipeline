#!/usr/bin/env bash
#
# submit_phase6.sh -- submit run_phase6.sbatch with account/QOS/partition/
...
# submit_phase4.sh/submit_phase5.sh read), so switching clusters or
# allocations is a config edit, not a script edit.
#
# Usage: workflow/scripts/submit_phase6.sh
...
echo "Submitting with: sbatch ${SBATCH_ARGS[*]} workflow/scripts/run_phase6.sbatch"
sbatch "${SBATCH_ARGS[@]}" workflow/scripts/run_phase6.sbatch
